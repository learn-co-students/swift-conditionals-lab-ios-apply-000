...
func isForceWith(name: String) -> Bool{
    let characterHasForce = ["Luke", "Leia", "Anakin", "Obi Wan", "Yoda", "Vader"]
    if characterHasForce.contains(name){
        return true
    }
    else {
        return false
...
*/
func bankAccounts(mine: Double, theirs: Double){
    if theirs - 10 >= 0 {
        theirs - 10
        mine + 10
    }
    else {
        print("No bueno, brother!")
    }
}

bankAccounts(mine: 20, theirs: 20)
bankAccounts(mine: 20, theirs: 9)
/*:
